...
if portal == "Student Portal":
    ...
    if st.session_state.get('student_id') is None:
        with st.form('student_login_form'):
            ...
            vcode = st.text_input("Verification code", max_chars=4, type="password")
        ...
        if student['year'] < 4:
            ...
            # Year 4 student - check graduation eligibility
            rf = student['locked_courses']
            has_remaining = rf and rf.strip() and 'none' not in rf.lower() and 'ready to graduate' not in rf.lower()
            is_paid = student['payment'] == 'Paid'
            
            if has_remaining:
                # Has remaining courses - allow registration
                available = [c.strip() for c in rf.split(',') if c.strip()]
                st.subheader("📚 Remaining Courses Available for Registration")
            elif not is_paid:
                # No remaining courses but fees unpaid
                available = []
                st.warning("⚠️ You have completed all courses, but your fees are unpaid!")
                st.error("💰 **Please pay your outstanding fees to complete graduation.**")
                st.info("Contact the finance office to settle your payment and receive your graduation clearance.")
            else:
                # No remaining courses and fees paid - GRADUATED! 🎉
                available = []
                
                # Celebration CSS for bubbles effect
                celebration_css = """
                <style>
                @keyframes float-up {
                ...
                """ + "".join([
                    f'<div class="bubble" style="left: {i*10}%; width: {20 + (i % 3) * 15}px; height: {20 + (i % 3) * 15}px; background: {"#FFD700" if i % 4 == 0 else "#87CEEB" if i % 4 == 1 else "#98FB98" if i % 4 == 2 else "#DDA0DD"}; animation-delay: {i * 0.3}s;"></div>'
                    for i in range(10)
                ]) + """
                </div>
                ...
                <div class="graduation-card">
                    ...
                    <p>You have successfully completed all requirements!</p>
                ...
                st.success("🎓 **You have officially graduated!**")
                st.info("📜 Please visit the registrar's office to collect your graduation documents and certificate.")
        ...
        if available:
            ...
            if completed:
                with st.expander("📚 Remaining Courses (Hidden from selection)"):
                    st.write(completed)
